...
aeEventLoop *aeCreateEventLoop(int setsize) { /* 创建一个EventLoop,我只是稍微有那么点好奇,究竟这些个玩意到底是怎么实现的. */
	...
	eventLoop->events = zmalloc(sizeof(aeFileEvent)*setsize); /* setsize是预设的大小吗?好吧,居然是一个数组 */
	...
	/* 初始化监听事件 */
	for (i = 0; i < setsize; i++)
		eventLoop->events[i].mask = AE_NONE; /* 表示不监听任何的事件,是吧. */
