...
/* 
* 如果字符串的大小小于REIDS_ENCODING_EMBSTR_SIZE_LIMIT,那么就构建一个EMBSTR编码的string对象,
* 否则的话,就使用RAW类型的编码.
*/
...
* 创建一个新 robj 对象
*/
robj *createObject(int type, void *ptr) { /* robj仅仅是对ptr的一个简单封装 */

	robj *o = zmalloc(sizeof(*o));

	o->type = type; /* type和编码是不同的,这里要记清楚 */
	o->encoding = REDIS_ENCODING_RAW; /* raw代表还没有初始化吗? */
	o->ptr = ptr;
	o->refcount = 1;
